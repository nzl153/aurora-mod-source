...
/// <summary>
/// 27 熔毁斩 / Meltdown Slash（罕见，A）。造成 10 伤；若打出前在过载区，改为造成 2 次，随后引爆。升级每段 10→12。
/// 顺序（§4.5）：开始只读一次区段 → 冷/温区 1 段 10/12；过载/临界区 2 段独立 10/12 后调标准 IgniteAsync。
/// 两段各自吃力量/易伤/过载×1.25/宕机/向下取整/消费本人锁定（中心补丁各段 +2），绝不合并为 20/24 单段。
