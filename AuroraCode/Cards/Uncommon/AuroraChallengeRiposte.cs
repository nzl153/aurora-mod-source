...
/// <summary>
/// 34 挑战反斩 / Challenge Riposte（罕见，B 协议兑现）。移除你施加给目标的全部挑战协议；造成 9 伤害，
/// 每实际移除 1 层额外造成 6 伤害；然后每实际移除 1 层最多散去 1 热。升级：基础伤害 9→12，每层额外伤害 6→7。
/// 从「偏防御的格挡兑现」改为<b>进攻兑现</b>——挑战协议本是「主动让敌人更能打你」的风险，
/// 兑现口理应是伤害而非再给盾（防御兑现由另一张协议牌承担）。给罕见位一张前期就能用的小终结技：满 3 层 27/33 伤。
...
/// 无本人协议时仍造成 9/12 基础伤害、不散热，不空牌。不产能量、不抽牌。人工制品/按施加者分层/目标死亡等边界继续走既有服务。
/// </summary>
public class AuroraChallengeRiposte() : AuroraCard(1, CardType.Attack, CardRarity.Uncommon, TargetType.AnyEnemy)
{
    ...
    [
        ...
        new AuroraScalingDamageVar(9, ValueProp.Move, (c, t) =>
